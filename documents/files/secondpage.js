...
var radiolevel =document.querySelectorAll(".radiolevel");
...
for (var i = 0; i < radiotheme.length; i++) {
    if (radiotheme[i].checked) {
        // insert code to use the checked value
        Pickedtheme=radiotheme[i].value;
        break;
    }
...
var levelvalue=()=>{
        for (var i = 0 ;i < radiolevel.length; i++) {
            if (radiolevel[i].checked) {
                // insert code to use the checked value
                pickedlevel=radiolevel[i].value;
                break;
           
            }
        }
        localStorage.setItem("radiolevel",pickedlevel);
        console.log("diff value is :",pickedlevel);
        }
...
radiolevel.forEach(item=>{
    item.addEventListener("change",levelvalue);
})
